...
python make.py                          # 25-step gate chain → build → contract (see below)
...
python make.py risk-pre-stamp           # escalateRisk() chokepoint (no direct findings.risk writes)
python make.py bare-ioc-types           # IOC.* constants only (no bare-string type: values)
python make.py pushioc-only             # pushIOC() chokepoint (no bare findings.*.push)
python make.py raw-text-lf              # lfNormalize() on every _rawText write
python make.py worker-spawn             # new Worker() allow-list (worker-manager + workers/)
python make.py silent-catch             # no empty catch {} in app-load / app-yara
python make.py storage-access           # safeStorage chokepoint (no direct localStorage)
python make.py extend-app               # extendApp() mixin chokepoint
python make.py backtick-comment         # no */ inside backtick spans in block comments
python make.py gate-tests               # Python unit tests for scripts/build/gates/
python make.py pipeline-sync            # make.py STEPS ↔ build.pipeline parity
python make.py release-test-api         # release bundle must not contain __loupeTest markers
python make.py fuzz-path-leak           # built bundle must not contain tests/fuzz/ paths
...
- Timeline can hold ≥1 "sources". A single-file load carries `_sources=null` and takes the legacy single-file path; dropping additional CSV/TSV/EVTX/structured-log/browser-history files on top (or a fresh multi-file drop where every file is merge-eligible) triggers `_timelineAddFile`, which builds a composite RowStore prefixed with `TIMELINE_CANONICAL_COLS` (the 9-entry trimmed schema: `__source`, `Timestamp`, `Host`, `User`, `EventID`, `Severity`, `Category`, `SourceIP`, `DestIP` — short identifier-shape values only, no `Process` / `Message` / `__format` slots; wide-narrative columns stay on each source's native plane, and format identity is conveyed by the source filename plus the per-chip format badge in the source-bar) and a `_sources` / `_sourceOfRow` / `_sourceEnabledBitmap` registry. Per-format projection lives in `src/app/timeline/timeline-mapper.js`; composite build in `src/app/timeline/timeline-composite.js`. Per-source colour (chip swatch, breadcrumb dot, `__source` cell tint) is derived from CURRENT array position via `timelineSourceColor(idx)` so all surfaces always share a hue and reshuffle in lockstep on add/remove. Browser-history SQLite is merge-ELIGIBLE (`'sqlite'`/`'db'` in `TIMELINE_MERGE_ELIGIBLE_KINDS`) but only for recognised browser DBs — `timelineSourceFromFile` throws `SQLITE_NOT_BROWSER` for a generic DB so the merge is refused with a toast (history projects only `Timestamp` into a canonical slot; its URL/Title/Domain/Type stay native). PCAP stays merge-INELIGIBLE (its `_pcapInfo` side-channel doesn't aggregate cleanly). For a single-source (n=1) composite view the canonical columns are hidden by default at mount via `_seedHiddenColumns`. Composite persistence is session-only (`TIMELINE_COMPOSITE_KEY_PREFIX`); per-source `_fileKey`s still work for solo reopen.
