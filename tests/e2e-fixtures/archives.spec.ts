...
import { gotoBundle, loadFixture, isRiskAtLeast } from '../helpers/playwright-helpers';
...
test.describe('archives renderer family', () => {
  ...
  test('plain ZIP enumerates entries', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.zip');
  ...
  test('encrypted ZIP surfaces an encryption Pattern row', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/encrypted-example.zip');
  ...
  test('AES-encrypted ZIP also escalates', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/aes-encrypted-example.zip');
    expect(findings.iocTypes).toContain('Pattern');
    expect(isRiskAtLeast(findings.risk, 'medium')).toBe(true);
  });

  test('7z archive parses without error', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.7z');
  ...
  test('CAB archive parses without error', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.cab');
    expect(findings.iocCount + findings.externalRefCount).toBeGreaterThan(0);
    expect(findings.yaraInProgress).toBe(false);
  });

  test('RAR archive parses without error', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.rar');
    // Listing-only RAR — entry names land as IOC.FILE_PATH.
    expect(findings.iocTypes).toContain('File Path');
    expect(findings.yaraInProgress).toBe(false);
  });

  test('ISO 9660 image enumerates entries', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.iso');
    expect(findings.iocTypes).toContain('File Path');
    expect(findings.yaraInProgress).toBe(false);
  });

  test('plain TAR archive enumerates entries', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.tar');
    expect(findings.iocTypes).toContain('File Path');
  });

  test('gzipped tar (.tar.gz) decompresses and enumerates inner TAR', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/archives/example.tar.gz');
    expect(findings.iocTypes).toContain('File Path');
  });

  test('recursive ZIP trips the aggregate budget guard', async ({ page }) => {
    ...
    // change wording.
    const findings = await loadFixture(page, 'examples/archives/recursive-example.zip');
  ...
  test('sha1-hulud worm fixture surfaces high risk', async ({ page }) => {
    // The `sha1-hulud.zip` fixture mirrors the npm worm shape used in
    // production red-team exercises. Risk floor must escalate.
    const findings = await loadFixture(page, 'examples/archives/sha1-hulud.zip');
