...
test.describe('forensics / Timeline-routed renderers', () => {
  ...
  async function assertTimelineLoaded(page, relPath: string) {
    const findings = await loadFixture(page, relPath);
  ...
  test('EVTX security log loads via Timeline', async ({ page }) => {
    await assertTimelineLoaded(page, 'examples/forensics/example-security.evtx');
  });

  test('EVTX system log loads via Timeline', async ({ page }) => {
    await assertTimelineLoaded(page, 'examples/forensics/example.evtx');
  });

  test('Chrome history SQLite loads via Timeline', async ({ page }) => {
    ...
    await assertTimelineLoaded(page, 'examples/forensics/chromehistory-example.sqlite');
  });

  test('generic SQLite falls back to regular analyser (zero-row escape)', async ({ page }) => {
    ...
    const findings = await loadFixture(page, 'examples/forensics/example.sqlite');
  ...
  test('JSON-shaped CSV loads via Timeline', async ({ page }) => {
    await assertTimelineLoaded(page, 'examples/forensics/json-example.csv');
  });
});
