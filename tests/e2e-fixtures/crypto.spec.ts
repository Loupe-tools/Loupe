...
test.describe('crypto / X.509 renderer', () => {
  ...
  test('PEM CA certificate (DER-encoded variant) parses without error', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example-ca.der');
    // Self-signed CA — at minimum the CA fact lands as a Pattern row.
    expect(findings.iocTypes).toContain('Pattern');
  });

  test('expired certificate flags expiry', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example-expired.crt');
  ...
  test('SAN-rich certificate enumerates host IOCs', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example-san.pem');
  ...
  test('self-signed certificate surfaces Pattern and Hostname rows', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example-selfsigned.pem');
    expect(findings.iocTypes).toContain('Pattern');
    expect(findings.iocTypes).toContain('Hostname');
    expect(isRiskAtLeast(findings.risk, 'medium')).toBe(true);
  });

  test('certificate-with-embedded-key escalates to critical', async ({ page }) => {
    ...
    const findings = await loadFixture(page, 'examples/crypto/example-with-key.pem');
    expect(isRiskAtLeast(findings.risk, 'critical')).toBe(true);
    expect(ruleNames(findings)).toContain('SSH_Private_Key_Reference');
  });

  test('Google certificate chain enumerates hostnames + URLs', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/google-chain.pem');
    expect(findings.iocTypes).toContain('Hostname');
    expect(findings.iocTypes).toContain('URL');
    expect(findings.detectionCount).toBeGreaterThan(0);
  });

  test('PKCS#12 (.p12) surfaces Pattern + Detection (regression)', async ({ page }) => {
    ...
    const findings = await loadFixture(page, 'examples/crypto/example.p12');
    expect(findings.detectionCount).toBeGreaterThan(0);
    expect(findings.externalRefCount).toBeGreaterThan(0);
    expect(findings.iocTypes).toContain('Pattern');
  });

  test('PKCS#12 (.pfx) surfaces Pattern + Detection (regression)', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example.pfx');
    expect(findings.detectionCount).toBeGreaterThan(0);
    expect(findings.externalRefCount).toBeGreaterThan(0);
    expect(findings.iocTypes).toContain('Pattern');
  });
...
test.describe('crypto / PGP renderer', () => {
  ...
  test('ASCII-armoured public key (.asc) parses', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example.asc');
    // The PGP renderer surfaces the user-id email as IOC.EMAIL.
    expect(findings.iocTypes).toContain('Email');
  });

  test('binary public key (.gpg) parses', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example.gpg');
    expect(findings.iocTypes).toContain('Email');
  });

  test('OpenPGP private key (.key) flags secret-key presence', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example.key');
  ...
  test('binary signature (.sig) parses without error', async ({ page }) => {
    // `.sig` files are detached signatures — there's no user-id and
    // no IOCs. We only assert no-crash.
    const findings = await loadFixture(page, 'examples/crypto/example.sig');
  ...
  test('ASCII-armoured PGP envelope (.pgp) parses', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/crypto/example.pgp');
    expect(findings.iocTypes).toContain('Email');
  });
});
