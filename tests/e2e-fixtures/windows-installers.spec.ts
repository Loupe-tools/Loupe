...
test.describe('Windows installers', () => {
  ...
  test('MSI installer parses with Pattern + Info rows', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/windows-installers/example.msi');
  ...
  test('MSIX package escalates to critical with capability rules', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/windows-installers/example.msix');
  ...
  test('AppInstaller XML escalates to critical with HTTP + auto-update rules', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/windows-installers/example.appinstaller');
    const rules = ruleNames(findings);
  ...
  test('benign ClickOnce .application parses with URL/IP', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/windows-installers/example.application');
    expect(findings.iocTypes).toContain('URL');
    expect(findings.iocTypes).toContain('IP Address');
  });

  test('ClickOnce manifest fires FullTrust_Requested', async ({ page }) => {
    const findings = await loadFixture(page, 'examples/windows-installers/example.manifest');
    expect(ruleNames(findings)).toContain('ClickOnce_FullTrust_Requested');
    expect(isRiskAtLeast(findings.risk, 'high')).toBe(true);
  });

  test('malicious ClickOnce escalates to critical with override + DDNS rules', async ({ page }) => {
    const findings = await loadFixture(
      page, 'examples/windows-installers/malicious-example.application');
    expect(isRiskAtLeast(findings.risk, 'critical')).toBe(true);
    const rules = ruleNames(findings);
