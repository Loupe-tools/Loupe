...
const ctx = loadModules(
  ...
  { expose: ['X509Renderer', 'PgpRenderer', 'IOC'] },
);

const { X509Renderer, PgpRenderer, IOC } = ctx;
...
for (const pin of PINS) {
  ...
  test(`crypto risk tier pin — ${label}`, () => {
    ...
    assert.equal(
      findings.riskScore,
      pin.riskScore,
      `${label} riskScore exact ${pin.riskScore} (got ${findings.riskScore})`,
    );
    ...
    assert.ok(
      findings.externalRefs.every(r => r && r.type === IOC.PATTERN),
      `${label} externalRefs must all be IOC.PATTERN rows`,
    );
  });
}