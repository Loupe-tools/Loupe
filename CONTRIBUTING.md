...
```bash
python make.py                   # default 25-step gate chain → build → contract
...
`chokepoint-apis`, `risk-pre-stamp`, `bare-ioc-types`, `pushioc-only`,
`raw-text-lf`, `worker-spawn`, `silent-catch`, `storage-access`,
`extend-app`, `backtick-comment`, `gate-tests`, `pipeline-sync`, `build`,
`release-test-api`, `fuzz-path-leak`, `contract`. JS concat order lives in
`scripts/build/js_sources.py` (`EARLY_JS_FILES`, `APP_JS_FILES`,
`_DETECTOR_FILES`). Direct `python scripts/build.py` runs pre-build gates via
`scripts/build/pipeline.py` derived from `make.py` (skip with
`LOUPE_SKIP_GATES=1`). Opt-in: `sbom`,
