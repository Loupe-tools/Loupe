...
| `loupe_ioc_worker_disabled` | string | "Off-thread IOC scan" checkbox in Settings tab (`_renderSettingsTab` in `src/app/app-settings.js`) | `"0"` (off — default; worker path enabled) or `"1"` (on; force synchronous in-tree path) | Forces the synchronous in-tree IOC mass-extract path even on non-timeline files larger than `IOC_WORKER_THRESHOLD_BYTES` (256 KB). Read at the dispatch site in `src/app/app-load.js`. Useful for A/B benchmarking and for analysts on Firefox `file://` who want predictable single-thread timing without the worker probe round-trip. Timeline-routed files (CSV / TSV / EVTX / SQLite browser-history) never reach the IOC pipeline regardless of this toggle — see the bypass invariant in `src/app/timeline/timeline-router.js:16-24`. |
