...
| CSV-export formula-injection defence (`_tlCsvCell`) | Untrusted forensic cell values beginning with `=` / `+` / `-` / `@` / TAB / CR are prefixed with `'` before RFC-4180 quoting, so a malicious log field cannot execute as a formula when a Timeline CSV export is opened in Excel / LibreOffice / Sheets |
