...
| **Merged multi-source Timelines** | Drop additional Timeline-eligible files on top of an existing Timeline — or drop several at once — to coalesce them into one chronologically-sorted view. The chip bar toggles each source on/off; 9 canonical columns (`__source`, `Timestamp`, `Host`, `User`, `EventID`, `Severity`, `Category`, `SourceIP`, `DestIP`) project across SIEM / audit / EDR vendor schemas so queries, pivots, and detections apply uniformly. Browser-history databases merge too — interleave a host EDR/CSV export with Chrome / Edge / Firefox history for triage (history contributes its `Timestamp` to the shared axis; its URL / Title / Domain / Type stay as native columns). Generic (non-browser) SQLite and PCAP can't merge — they open standalone. |
...
| **Injection-safe CSV export** | Every Timeline CSV export (grid, top-value columns, chart, pivot) is RFC 4180-quoted **and** spreadsheet-formula-defanged: any cell whose value starts with `=`, `+`, `-`, `@`, TAB, or CR is prefixed with a `'` so untrusted forensic data can't execute as a formula when the file is opened in Excel / LibreOffice / Sheets. |
