...
| **Encoded content detection** | Scans for Base64, hex, Base32, gzip / zlib / deflate, JS `\xHH` hex escapes, char-array `String.fromCharCode` chains, reverse-string transforms (JS / PowerShell / Python `reversed()`), token-spaced obfuscation (`W  r  i  t  e  -  O  u  t  p  u  t`), and literal string-concat assembly. Decodes, classifies the payload (PE, script, URL list, …), extracts IOCs, and offers "Load for analysis" to drill into the decoded layer. |
