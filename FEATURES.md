...
| **macOS Installers** | `.dmg` (Apple Disk Image / UDIF — `koly` trailer, partition / mish block enumeration, AEA1 / `encrcdsa` / `cdsaencr` encrypted envelope detection, embedded `.app` bundle path extraction) · `.pkg` `.mpkg` (flat PKG / xar archive — zlib-decompressed TOC, `Distribution` / `PackageInfo` XML, clickable entry drill-down, dangerous-script-name flagging: `preinstall` / `postinstall` / `preflight` / `postflight` / `InstallationCheck` / `VolumeCheck`) — 12 YARA rules for macOS installer threats |
