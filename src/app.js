...
class DocxParser {
  ...
  async _extractMacros(zip) {
    ...
    try {
      ...
      // rawBin is kept so _downloadMacros can offer a binary download even when
      // text decoding fails to produce readable source (Fix 1).
      return {present:true, size:data.length, sha256:await this._sha256(data), modules:this._parseVBA(data), rawBin:data};
...
class SecurityAnalyzer {
  analyze(parsed) {
    const f={hasMacros:false,autoExec:[],externalRefs:[],modules:[],metadata:{},risk:'low',macroSize:0,macroHash:null,rawBin:null};
    ...
    if(parsed.macros?.present){
      ...
      // Fix 2: carry the raw binary forward so _downloadMacros can offer it
      // even when text decoding produced no readable source.
  ...
  _externalRefs(parsed) {
    ...
    // Fix 3: Also scan w:instrText nodes for HYPERLINK field codes.
    // Some documents embed hyperlinks via field codes (e.g. HYPERLINK "https://…")
    // that never appear as External relationships in the rels file.
    if(parsed.document){
      ...
      try {
        for(const instr of Array.from(parsed.document.getElementsByTagNameNS(W,'instrText'))){
          ...
          seen.add(url);
          // Only report HTTP/HTTPS/mailto as info; anything else (file://, UNC, etc.) is medium
          const safe=sanitizeUrl(url);
          refs.push({type:'Hyperlink (field code)',url:url,severity:safe?'info':'medium'});
        }
      } catch(e){}
    }
    return refs;
...
// ── Compact pure-JS MD5 (no crypto.subtle — not supported for MD5) ───────────
function _md5(bytes) {
...
class App {
  ...
  async _loadFile(file) {
    ...
    try {
      ...
      const hashPromise=this._hashFile(buffer,file.name);
  ...
  _sec(label){const d=document.createElement('div');d.className='sb-section';d.textContent=label;return d;}
  ...
  _fmtBytes(b){if(!b||b<1024)return(b||0)+' B';if(b<1048576)return(b/1024).toFixed(1)+' KB';return(b/1048576).toFixed(1)+' MB';}
}
