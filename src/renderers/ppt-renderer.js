...
class PptBinaryRenderer {
  ...
  analyzeForSecurity(buffer) {
    ...
    try {
      ...
      const vbaDir = cfb.get('_VBA_PROJECT_CUR/VBA/dir') || cfb.get('VBA/dir');
      const vbaProj = cfb.get('_VBA_PROJECT_CUR/vbaProject.bin') || cfb.get('VBA/vbaProject.bin');
      if (vbaDir || vbaProj) {
        f.hasMacros = true; f.risk = 'medium';
      }

      // Try to extract VBA modules
      for (const [name, data] of cfb.entries()) {
      ...
      const vbaProjBin = cfb.get('_VBA_PROJECT_CUR/VBA') || cfb.get('_VBA_PROJECT_CUR');
      if (f.hasMacros) {
      ...
      const summaryInfo = cfb.get('\x05SummaryInformation') || cfb.get('SummaryInformation');
      if (summaryInfo) {
        try {
  ...
  _extract(cfb) {
    ...
    const pptDoc = cfb.get('PowerPoint Document') || cfb.get('PowerPoint_Document');
