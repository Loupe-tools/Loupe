...
def generate_markdown(js_infos, css_infos, yar_infos, cross_refs):
    ...
    w("   'use strict';")
    w('   class FooRenderer {')
    w('     static async render(file, arrayBuffer, app) { /* ... */ }')
    w('   }')
    w('   ```')
    ...
    w('5. Run `python build.py` and test with a sample file.')
    w('6. Run `python generate-codemap.py` to update this map.')
    w('')
